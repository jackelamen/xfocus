...
function pxToMins(px) { return Math.round(px / PX_PER_MIN) + HOUR_START * 60 }
...
function TimeBlock({ block, onEdit, onFocusNow }) {
  ...
  return (
    <div
      ...
      style={{ top, height, background: block.color + '22', border: `1.5px solid ${block.color}55`, zIndex: 5 }}
    >
      ...
      <div className="pl-3 pr-2 py-1.5 h-full flex flex-col justify-between">
        <div className="flex items-start justify-between gap-1">
          <div className="flex-1 min-w-0">
            <p
              ...
              style={{ color: block.color, fontFamily: 'Inter, sans-serif', fontSize: height < 36 ? 9 : 11 }}
            >
              {block.title}
            </p>
            {height > 40 && block.task_names?.length > 0 && (
              <p className="text-[9px] truncate mt-0.5" style={{ color: 'rgba(255,255,255,0.45)' }}>
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button
              ...
              className="w-5 h-5 rounded flex items-center justify-center text-white transition-all"
            ...
            <button
              ...
              className="w-5 h-5 rounded flex items-center justify-center text-white/50 hover:text-white transition-colors"
              title="Edit"
            >
        ...
        {height > 55 && (
          <p className="text-[9px] font-bold" style={{ color: 'rgba(255,255,255,0.3)' }}>
            {block.start_time.slice(0, 5)} – {block.end_time.slice(0, 5)}
          </p>
        )}
        {block.completed && (
          <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
            <span className="material-symbols-rounded text-emerald-400" style={{ fontSize: 20 }}>check_circle</span>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        background: isOver ? 'rgba(249,115,22,0.08)' : 'transparent',
        border: isOver ? '1px dashed rgba(249,115,22,0.4)' : '1px dashed transparent',
        borderRadius: 8,
        zIndex: 1,
      }}
    />
...
export default function BlocksPage({ user }) {
  ...
  const { tasks: pulseTasks } = usePulseTasks(user.id)

  const [formBlock, setFormBlock] = useState(null)   // null=closed, {}=new, block=editing
  ...
  function handleDragStart({ active }) {
    if (active.data.current?.type === 'task') {
      setActiveDrag(active.data.current.task)
    }
  }
  ...
  async function handleDragEnd({ active, over }) {
    ...
    if (overId.startsWith('block-')) {
      // Drop onto existing block
      const blockId = overId.replace('block-', '')
      await addTaskToBlock(blockId, task.id, task.title)
    } else if (overId.startsWith('slot-')) {
      // Drop onto empty hour slot — create a new block
    ...
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="h-screen flex flex-col overflow-hidden" style={{ background: '#0f0f1a' }}>
        ...
        <div
          ...
          style={{ background: '#12121f', borderBottom: '1px solid rgba(255,255,255,0.06)' }}
        >
          <div>
            <h2 className="font-black text-white" style={{ fontFamily: 'Manrope, sans-serif', fontSize: 17 }}>Time Blocks</h2>
            ...
            <div className="flex rounded-xl p-1 gap-1" style={{ background: 'rgba(255,255,255,0.05)' }}>
              {[{ label: 'Today', val: todayStr() }, { label: 'Tomorrow', val: tomorrowStr() }].map(({ label, val }) => (
                <button key={val} onClick={() => setViewDate(val)}
                  ...
                  style={viewDate === val ? { background: '#f97316', color: '#fff' } : { color: 'rgba(255,255,255,0.35)' }}
                >
                  {label}
                </button>
              ))}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto" ref={timelineRef}>
            <div className="flex" style={{ minHeight: TIMELINE_HEIGHT + 64 }}>
              ...
              <div className="flex-shrink-0 select-none" style={{ width: 56, paddingTop: 8, paddingBottom: 32 }}>
                {hours.map(h => (
                  ...
                  >
                    <span className="text-[10px] font-bold tabular-nums" style={{ color: 'rgba(255,255,255,0.2)', marginTop: -6 }}>
              ...
              <div className="flex-1 relative" style={{ paddingRight: 16, paddingTop: 8, paddingBottom: 32, height: TIMELINE_HEIGHT + 40 }}>
                ...
                {hours.map(h => (
                  <div
                    key={h}
                    className="absolute left-0 right-4 pointer-events-none"
                    style={{ top: minsToPx(h * 60), height: 1, background: 'rgba(255,255,255,0.05)' }}
                  />
                ))}
      ...
      <DragOverlay>
        {activeDrag && (
          <div
            className="px-3 py-2 rounded-xl text-xs font-bold text-white shadow-xl"
            style={{ background: '#f97316', maxWidth: 200, transform: 'rotate(2deg)' }}
          >
            {activeDrag.title}
          </div>
        )}
      ...
      {formOpen && (
        <BlockForm
          user={user}
          initial={formBlock}
          onClose={() => { setFormOpen(false); setFormBlock(null) }}
        />
      )}
    </DndContext>
  )
